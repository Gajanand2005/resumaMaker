...
const ResumaBuilder = () => {
  ...
  const handleShare=() =>{
    ...
    if (navigator.share) {
      ...
      alert("Your browser does not support sharing. You can copy the link manually: ")
    }
  }
  ...
  return (
    ...
    <div className='flex justify-between items-center mb-6 border-b border-gray-300 py-1'>
      ...
      <div className='flex items-center'>
        ...
        <button className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${activeSectionIndex === section.length - 1  && 'opacity-50'}`} disabled={activeSectionIndex === section.length - 1}  onClick={()=>setActiveSectionIndex((prevIndex)=>Math.min(prevIndex + 1, section.length - 1))}>
      ...
      <div className='lg:col-span-7 max-lg:mt-6'>
          ...
          {resumeData.public && (
            <button onClick={handleShare}  className='flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-blue-100 to-blue-100 text-blue-600 rounded-lf ring-blue-300 hover:ring transition-colors '>
              <Share2Icon className='size-4' /> Share
            </button>
          )}
          <button onClick={changeResumeVisibility} className='flex items-center p-2 px-4 gap-2 text-xs bg-gradient-to-br from-purple-100 to-purple-200 text-purple-600 ring-purple-300 rounded-lf hover:ring transition-colors'>
